...
type InputMedia = inputMediaEmpty | inputMediaUploadedPhoto | inputMediaPhoto | inputMediaGeoPoint | inputMediaContact | inputMediaUploadedDocument | inputMediaDocument | inputMediaVenue | inputMediaPhotoExternal | inputMediaDocumentExternal | inputMediaGame | inputMediaInvoice | inputMediaGeoLive | inputMediaPoll | inputMediaDice;
...
type InputFileLocation = inputFileLocation | inputEncryptedFileLocation | inputDocumentFileLocation | inputSecureFileLocation | inputTakeoutFileLocation | inputPhotoFileLocation | inputPhotoLegacyFileLocation | inputPeerPhotoFileLocation | inputStickerSetThumb | inputGroupCallStream;
...
type MessageMedia = messageMediaEmpty | messageMediaPhoto | messageMediaGeo | messageMediaContact | messageMediaUnsupported | messageMediaDocument | messageMediaWebPage | messageMediaVenue | messageMediaGame | messageMediaInvoice | messageMediaGeoLive | messageMediaPoll | messageMediaDice;
...
type PhotoSize = photoSizeEmpty | photoSize | photoCachedSize | photoStrippedSize | photoSizeProgressive | photoPathSize;
...
type ReportReason = inputReportReasonSpam | inputReportReasonViolence | inputReportReasonPornography | inputReportReasonChildAbuse | inputReportReasonOther | inputReportReasonCopyright | inputReportReasonGeoIrrelevant | inputReportReasonFake | inputReportReasonIllegalDrugs | inputReportReasonPersonalDetails;
...
type ExportedChatInvite = chatInviteExported;
type ChatInvite = chatInviteAlready | chatInvite | chatInvitePeek;
type InputStickerSet = inputStickerSetEmpty | inputStickerSetID | inputStickerSetShortName | inputStickerSetAnimatedEmoji | inputStickerSetDice | inputStickerSetAnimatedEmojiAnimations;
type StickerSet = stickerSet;
type messages_StickerSet = messages_stickerSet | messages_stickerSetNotModified;
...
type ChannelParticipant = channelParticipant | channelParticipantSelf | channelParticipantCreator | channelParticipantAdmin | channelParticipantBanned | channelParticipantLeft;
type ChannelParticipantsFilter = channelParticipantsRecent | channelParticipantsAdmins | channelParticipantsKicked | channelParticipantsBots | channelParticipantsBanned | channelParticipantsSearch | channelParticipantsContacts | channelParticipantsMentions;
...
type InputBotInlineMessage = inputBotInlineMessageMediaAuto | inputBotInlineMessageText | inputBotInlineMessageMediaGeo | inputBotInlineMessageMediaVenue | inputBotInlineMessageMediaContact | inputBotInlineMessageGame | inputBotInlineMessageMediaInvoice;
...
type BotInlineMessage = botInlineMessageMediaAuto | botInlineMessageText | botInlineMessageMediaGeo | botInlineMessageMediaVenue | botInlineMessageMediaContact | botInlineMessageMediaInvoice;
...
type auth_CodeType = auth_codeTypeSms | auth_codeTypeCall | auth_codeTypeFlashCall | auth_codeTypeMissedCall;
type auth_SentCodeType = auth_sentCodeTypeApp | auth_sentCodeTypeSms | auth_sentCodeTypeCall | auth_sentCodeTypeFlashCall | auth_sentCodeTypeMissedCall;
type messages_BotCallbackAnswer = messages_botCallbackAnswer;
type messages_MessageEditData = messages_messageEditData;
type InputBotInlineMessageID = inputBotInlineMessageID | inputBotInlineMessageID64;
...
type InputPaymentCredentials = inputPaymentCredentialsSaved | inputPaymentCredentials | inputPaymentCredentialsApplePay | inputPaymentCredentialsGooglePay;
...
type PhoneConnection = phoneConnection | phoneConnectionWebrtc;
...
type InputMessage = inputMessageID | inputMessageReplyTo | inputMessagePinned | inputMessageCallbackQuery;
...
type auth_LoginToken = auth_loginToken | auth_loginTokenMigrateTo | auth_loginTokenSuccess;
type account_ContentSettings = account_contentSettings;
type messages_InactiveChats = messages_inactiveChats;
...
type messages_VotesList = messages_votesList;
type BankCardOpenUrl = bankCardOpenUrl;
type payments_BankCardData = payments_bankCardData;
type DialogFilter = dialogFilter;
type DialogFilterSuggested = dialogFilterSuggested;
type StatsDateRangeDays = statsDateRangeDays;
type StatsAbsValueAndPrev = statsAbsValueAndPrev;
type StatsPercentValue = statsPercentValue;
type StatsGraph = statsGraphAsync | statsGraphError | statsGraph;
type MessageInteractionCounters = messageInteractionCounters;
type stats_BroadcastStats = stats_broadcastStats;
type help_PromoData = help_promoDataEmpty | help_promoData;
type VideoSize = videoSize;
type StatsGroupTopPoster = statsGroupTopPoster;
type StatsGroupTopAdmin = statsGroupTopAdmin;
type StatsGroupTopInviter = statsGroupTopInviter;
type stats_MegagroupStats = stats_megagroupStats;
type GlobalPrivacySettings = globalPrivacySettings;
type help_CountryCode = help_countryCode;
type help_Country = help_country;
type help_CountriesList = help_countriesListNotModified | help_countriesList;
type MessageViews = messageViews;
type messages_MessageViews = messages_messageViews;
type messages_DiscussionMessage = messages_discussionMessage;
type MessageReplyHeader = messageReplyHeader;
type MessageReplies = messageReplies;
type PeerBlocked = peerBlocked;
type stats_MessageStats = stats_messageStats;
type GroupCall = groupCallDiscarded | groupCall;
type InputGroupCall = inputGroupCall;
type GroupCallParticipant = groupCallParticipant;
type phone_GroupCall = phone_groupCall;
type phone_GroupParticipants = phone_groupParticipants;
type InlineQueryPeerType = inlineQueryPeerTypeSameBotPM | inlineQueryPeerTypePM | inlineQueryPeerTypeChat | inlineQueryPeerTypeMegagroup | inlineQueryPeerTypeBroadcast;
type messages_HistoryImport = messages_historyImport;
type messages_HistoryImportParsed = messages_historyImportParsed;
type messages_AffectedFoundMessages = messages_affectedFoundMessages;
type ChatInviteImporter = chatInviteImporter;
type messages_ExportedChatInvites = messages_exportedChatInvites;
type messages_ExportedChatInvite = messages_exportedChatInvite | messages_exportedChatInviteReplaced;
type messages_ChatInviteImporters = messages_chatInviteImporters;
type ChatAdminWithInvites = chatAdminWithInvites;
type messages_ChatAdminsWithInvites = messages_chatAdminsWithInvites;
type messages_CheckedHistoryImportPeer = messages_checkedHistoryImportPeer;
type phone_JoinAsPeers = phone_joinAsPeers;
type phone_ExportedGroupCallInvite = phone_exportedGroupCallInvite;
type GroupCallParticipantVideoSourceGroup = groupCallParticipantVideoSourceGroup;
type GroupCallParticipantVideo = groupCallParticipantVideo;
type stickers_SuggestedShortName = stickers_suggestedShortName;
type BotCommandScope = botCommandScopeDefault | botCommandScopeUsers | botCommandScopeChats | botCommandScopeChatAdmins | botCommandScopePeer | botCommandScopePeerAdmins | botCommandScopePeerUser;
type account_ResetPasswordResult = account_resetPasswordFailedWait | account_resetPasswordRequestedWait | account_resetPasswordOk;
type SponsoredMessage = sponsoredMessage;
type messages_SponsoredMessages = messages_sponsoredMessages;
type SearchResultsCalendarPeriod = searchResultsCalendarPeriod;
type messages_SearchResultsCalendar = messages_searchResultsCalendar;
type SearchResultsPosition = searchResultPosition;
type messages_SearchResultsPositions = messages_searchResultsPositions;
type channels_SendAsPeers = channels_sendAsPeers;
type users_UserFull = users_userFull;
type messages_PeerSettings = messages_peerSettings;
type auth_LoggedOut = auth_loggedOut;
type ReactionCount = reactionCount;
type MessageReactions = messageReactions;
type messages_MessageReactionsList = messages_messageReactionsList;
type AvailableReaction = availableReaction;
type messages_AvailableReactions = messages_availableReactionsNotModified | messages_availableReactions;
type messages_TranslatedText = messages_translateNoResult | messages_translateResultText;
type MessagePeerReaction = messagePeerReaction;
type GroupCallStreamChannel = groupCallStreamChannel;
type phone_GroupCallStreamChannels = phone_groupCallStreamChannels;
type phone_GroupCallStreamRtmpUrl = phone_groupCallStreamRtmpUrl;
...
// Auto-generated with https://github.com/VityaSchel/mtproto-core-typescript-codegen in 0.069s